% ============================================================
% KAPITEL 3 - METHODISCHES VORGEHEN - KORRIGIERTE FASSUNG
% Nur Rechtschreibung, Grammatik und Vereinheitlichung - keine
% inhaltlichen Änderungen. Eure neuesten inhaltlichen Änderungen
% (Verweis "(siehe 3.3.1)" bei der Kabine, gestrichener Absatz zum
% Innen-/Außen-Vergleich) wurden unverändert übernommen.
%
% VEREINHEITLICHUNGEN:
% - "EKP" -> "ERP" (Kapitel 1 und 2 nutzen durchgängig "ERP"; nur hier
%   stand einmalig "EKP" über \acrlong{ekp} - für Konsistenz angepasst.
%   Falls ihr \newacronym{ekp}{...} definiert habt, wird dieser Eintrag
%   dadurch nirgends mehr verwendet - bitte prüfen, ob ihr EKP ganz
%   streichen oder doch behalten wollt.)
% - "BioSemi Active II-System" -> "BioSemi ActiveTwo-System" (Kapitel 1
%   nennt das Gerät "BioSemi ActiveTwo")
% - "Ultracortex Mark IV" bereits konsistent mit Kapitel 1
% - "Gruppenmittlung" -> "Gruppenmittelung" (durchgängig)
% - Zahlenbereiche einheitlich mit Halbgeviertstrich (--)
%
% Die zwei hartkodierten Verweise "Abschnitt 2.4.6" und "Abschnitt 2.4.7"
% wurden NICHT verändert (siehe ANMERKUNG unten), da eine Korrektur der
% Nummern eine Prüfung der aktuellen Kapitel-2-Gliederung erfordert und
% ich hier keine Zahl raten möchte.
% ============================================================
...
\textbf{Epochierung:} Zunächst wird die fortlaufende Aufzeichnung jeder Versuchsperson um jeden einzelnen Bildreiz herum in kurze Ausschnitte, sogenannte Epochen, zerschnitten. Jede Epoche beginnt kurz vor dem jeweiligen Reizbeginn und reicht bis knapp eine Sekunde danach, sodass sowohl ein Referenzzeitraum vor dem Reiz als auch der gesamte relevante Antwortverlauf danach erfasst wird. Die beiden Darbietungsbedingungen groß/normalkontrastiert und klein/kontrastreduziert (vgl. Abschnitt 3.3.1) werden dabei von Anfang an getrennt gehalten, da sie später getrennt gemittelt und miteinander verglichen werden sollen. In diesem Schritt findet noch keine Basislinienkorrektur statt; sie wird bewusst erst nach der Artefaktbereinigung durchgeführt (siehe unten).
