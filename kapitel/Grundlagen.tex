...
Das Gehirn ist das zentrale Organ des menschlichen Nervensystems und gehört zum zentralen Nervensystem.\footnote{Vgl. \cite{hescheler2010}} Es ist über das \Gls{rückenmark} und das \Gls{peripheres Nervensystem} mit dem ganzen Körper verbunden und übernimmt Funktionen, wie die Verarbeitung von Informationen, die Integration von Sinnesreizen, die Koordination von Bewegungen und die Regulierung lebensnotwendiger Prozesse.\footnote{Vgl. \cite{wiki_gehirn}} Die zuvor behandelten Strukturen des Gehirns bilden dabei die Grundlage für diese Funktionen. Damit diese Strukturen aktiv miteinander arbeiten können, ist eine Kommunikation notwendig. \newline
...
Das Aktionspotenzial wird entlang des Axons weitergeleitet und ermöglicht so die schnelle Informationsübertragung innerhalb des Neurons. Um Informationen weiterzuleiten, ist eine Übertragung an Synapsen notwendig. Eine Synapse ist eine Kontaktstruktur, eine Verbindungsstelle, zur Übertragung eines chemischen oder elektrischen Signals einer Nervenzelle auf eine andere Nervenzelle. Die Synapse ist aus einer Präsynapse, der Postsynapse und dem dazwischenliegenden synaptischen Spalt aufgebaut.\footnote{Vgl. \cite{doccheck_synapse}} \newline
...
Jedes Neuron ist mit einer Vielzahl von Synapsen mit anderen Neuronen verbunden. Somit treffen in einem Neuron kontinuierlich zahlreiche Signale über Synapsen ein, die darüber entscheiden, ob ein Aktionspotenzial ausgelöst wird oder nicht. Diese Signale sind entweder erregend oder hemmend. Erregende Signale werden als exzitatorische postsynaptische Potenziale (EPSPs) bezeichnet und hemmende Signale als inhibitorische postsynaptische Potenziale (IPSPs).\footnote{Vgl. \cite{antwerpes_summation}} Ein einzelnes EPSP reicht in der Regel nicht aus, um den Schwellenwert für die Auslösung eines Aktionspotenzials zu überschreiten. Daher ist eine Verrechnung mehrerer Signale notwendig. Diese Verrechnung erfolgt am Axonhügel des Neurons, dort werden die EPSPs und IPSPs summiert. Übersteigt diese Summe einen bestimmten Schwellenwert, unter Berücksichtigung der hemmenden Signale, wird ein Aktionspotenzial am Axonhügel ausgelöst.\footnote{Vgl. Ebd.}\newline

Dabei unterscheidet man zwei Formen der Summation: die zeitliche und die räumliche Summation. Bei der zeitlichen Summation addieren sich mehrere IPSPs oder EPSPs, die in kurzen Abständen an die Synapse übertragen werden. Wenn sich IPSPs zeitlich summieren, wird das Membranpotenzial des folgenden Neurons zunehmend negativer (Hyperpolarisation). Das bedeutet, dass sich das Membranpotenzial weiter vom Schwellenwert entfernt.\footnote{Vgl. \cite{examio_summation}} Hingegen, wenn sich EPSPs zeitlich summieren, wird das Membranpotenzial des folgenden Neurons zunehmend positiver (Depolarisation). Dadurch steigt die Chance, dass der Schwellenwert für ein Aktionspotenzial übertroffen wird.\footnote{Vgl. Ebd.} Die zweite Form der Summation ist die räumliche Summation, diese beschreibt das gleichzeitige Eintreffen von Signalen an verschiedenen Synapsen. Dabei werden mehrere EPSPs oder auch eine Mischung aus EPSPs und IPSPs gleichzeitig verrechnet.\footnote{Vgl. Ebd.} Ausschlaggebend dabei ist die Entfernung der Synapse zum Axonhügel. Signale, die näher am Axonhügel übertragen werden, haben einen stärkeren Einfluss auf die Entstehung eines Aktionspotenzials. Beispielsweise treffen entfernt vom Axonhügel viele EPSPs ein, während nahe am Axonhügel ein IPSP eintrifft, in diesem Fall kann es passieren, dass der Schwellenwert nicht überschritten wird. Zusammenfassend lässt sich sagen, dass die Entstehung eines Aktionspotenzials das Ergebnis einer komplexen Verrechnung von erregenden und hemmenden Einflüssen ist. Nur wenn die Summe der EPSPs die hemmenden Effekte der IPSPs überwiegt und der Schwellenwert am Axonhügel überschritten wird, kommt es zur Weiterleitung eines elektrischen Signals entlang des Axons.\footnote{Vgl. \cite{antwerpes_summation}}
...
Ein wichtiger Anwendungsbereich sind sogenannte \Glspl{brain-computer-interface} (BCI, Mehrzahl BCIs). Hierbei wird die Gehirnaktivität erfasst, analysiert und in Steuersignale umgewandelt. Systeme wie das von NeuSen W wurden gezielt für diesen Zweck entwickelt. BCIs stellen somit eine konkrete Anwendung der Neurotechnik dar.
...
Im militärischen Bereich werden EEGs beispielsweise zur Beurteilung von Schädel-Hirn-Traumata infolge von Unfällen eingesetzt.\footnote{Vgl. \cite{fanter1986}} \newline
...
Um einen Vergleich zwischen EEG-Geräten zu ermöglichen, werden sie grundsätzlich nach drei Kriterien unterschieden, den Elektroden, der Art der Übertragung zur Weiterverarbeitung und dem Anwendungsbereich. \newline
...
Das letzte Kriterium zur Einordnung von Geräten, mit denen EEGs durchgeführt werden, ist der Anwendungsbereich. Die größten Bereiche sind Forschung und Kliniken. Die Geräte, die zur Forschung genutzt werden, besitzen mehr Elektroden und das Ziel ist eine hohe Präzision, um neue Erkenntnisse zu gewinnen. In Kliniken werden EEGs nach klinischen Standards durchgeführt und für medizinische Diagnosen und Überwachungen, auch über lange Zeiträume, genutzt. Dabei ist die Bequemlichkeit wichtiger als in der Forschung.\footnote{Vgl. Ebd., S. 1 und 2.}

\subsection{Allgemeiner Aufbau der Geräte}
EEG-Geräte bestehen aus vier grundlegenden Komponenten: Elektroden, Vorverarbeitungsschaltungen, \acrshort{adc}s (kurz für \Gls{analog-digital-converter}) und Signalsteuerung und -feedback.\footnote{ Vgl. Ebd., S. 4}
Elektroden werden in drei Kategorien unterteilt: invasiv, nicht-invasiv-nass und nicht-invasiv-trocken.\newline
Die Nutzung von nicht-invasiven Elektroden ist ein sicherer und chirurgiefreier Ansatz. Beispiele für solche Elektroden sind Mikronadeln (Abbildung \ref{Spitze Elektroden}), die leicht in die Haut einstechen und so den Neuronen näher sind, und fingerförmige Elektroden (Abbildung \ref{Fingerelektroden}), die leicht durchs Haar dringen und angenehmer sind als Mikronadeln. Beide üben keinen Druck auf den Kopf aus und liegen direkt am Kopf an, was die Stabilität und Genauigkeit erhöht. Die elektrischen Signale sind jedoch gestreut und geschwächt, bevor sie gemessen werden, was die Qualität der aufgenommenen Daten verringert.\footnote{ Vgl. Ebd., S. 4 und 5}\newline
Trockene Elektroden bestehen aus einem leitfähigen Material wie Metall und liegen eng am Kopf an. Bei nassen Elektroden wird ein flüssiges \Gls{elektrolyt} verwendet, wie eine Salzlösung oder ein leitfähiges Gel. Das Elektrolyt sorgt für eine bessere elektrische Verbindung zwischen den Elektroden und der Kopfhaut, also weniger Widerstand, kann jedoch austrocknen und ist deswegen nicht für langfristige Nutzung geeignet.\footnote{ Vgl. \cite{anchuang_elektrodenkappe}} \newline
...
Die Filter eliminieren ungewollte Störungen, behalten jedoch die Hauptfrequenz bei. Verstärker werden eingesetzt, um die geringen Amplituden des Signals (das Rohsignal liegt im Mikrovoltbereich) zu erhöhen (zum Beispiel in den Millivoltbereich), dadurch wird es einfacher, sie digital aufzuzeichnen.\footnote{ Vgl. \cite{qin2023}, S. 8} Referenzelektroden sind an Stellen des Kopfes angebracht, an denen keine Gehirnaktivität zu messen ist und machen einen Spannungsvergleich mit den anderen Elektroden möglich. Die Groundelektrode an Stirn oder Wange misst nicht nur sehr wenig Hirnaktivität, sondern schafft ein gemeinsames elektrisches Bezugspotenzial für das Messsystem. Wenn sich elektromagnetische Felder in der Umgebung befinden, beispielsweise aufgrund eines Monitors, der neben der Messanordnung steht, misst die Groundelektrode die dadurch entstandene Spannung. Das Zusammenspiel beider Elektroden, Referenz und Ground, ermöglicht Berechnungen in der Vorverarbeitungsschaltung. Die von dort weitergegebenen Daten zeigen einzig die Aktivität des Gehirns.\footnote{ Vgl. \cite{biopac_reference_ground}}\newline
...
\section{Auswertung der durch EEGs erzeugten Daten - Frequenzbandanalyse und ereignisbezogene Potenziale}
Sophie \newline
...
Es wird erstens in spontane EEG-Rhythmen unterschieden, welche als kontinuierliche Oszillationen in verschiedenen Frequenzbereichen beobachtet werden können, und zweitens in ERPs (aus dem Englischen \emph{event-related potentials}, \glqq ereignisbezogene Potenziale\grqq), welche als zeitlich gebundene Spannungsänderungen in Reaktion auf bestimmte Ereignisse oder Stimuli erscheinen.\footnote{Vgl. Ebd.} Beide Perspektiven unterscheiden sich also grundlegend in ihrer zeitlichen Struktur: Während EEG-Rhythmen kontinuierlich und ereignisunabhängig sind, sind ERPs kurzzeitig, gebunden an ein konkretes Ereignis und nur durch Mittelung vieler Wiederholungen im Experiment erkennbar. Zusammen erlauben beide Perspektiven die Analyse der EEG-Daten. \newline


Genau diese eben beschriebenen Perspektiven bilden die methodische Grundlage für die im Rahmen unserer Arbeit durchgeführte Validierungsstudie. Um die Validität des portablen EEG-Systems zu bewerten, müssen deshalb beide Betrachtungsweisen bedacht werden. Aus diesem Grund wird zum einen die \textbf{Alpha-Power} stellvertretend für die spontanen Rhythmen herangezogen, zum anderen die \textbf{P1-Komponente} stellvertretend für die ereignisbezogenen Potenziale. \newline 
Im Folgenden werden deshalb zunächst die Frequenzbandbereiche (Abschnitt \ref{sec:frequenzbaender}) und anschließend die ERPs (Abschnitt \ref{sec:erps}) erläutert, wobei ein besonderer Fokus auf die in unserer Studie gewählten Vergleichsgrößen gelegt wird. Alle weiteren Bänder bzw. Komponenten können, ausführlicher beschrieben, auch im Anhang gefunden werden (vgl. \ref{app:frequenzbaender} und \ref{app:erp}). Abschließend werden auch allgemeine Störgrößen behandelt, die später bei der Verarbeitung der in der Studie aufgenommenen Rohdaten beachtet werden müssen (Abschnitt \ref{sec:stoergroessen}). 

% ANMERKUNG: \ref{app:frequenzbaender} und \ref{app:erp} verweisen aktuell
% auf kein vorhandenes \label (kein Anhang in den mir vorliegenden Dateien) -
% das ergibt beim Kompilieren "??". Sobald der Anhang existiert, bitte dort
% die entsprechenden \label{app:frequenzbaender} bzw. \label{app:erp} setzen.

\subsection{EEG-Rhythmen in verschiedenen Frequenzbandbereichen - Frequenzbandanalyse}
\label{sec:frequenzbaender}

EEG-Signale enthalten rhythmische Oszillationen, welche historisch in verschiedene Frequenzbänder unterteilt wurden, die mit griechischen Buchstaben bezeichnet wurden. Diese inkludieren Delta ($\delta$)-, Theta ($\theta$)-, Alpha ($\alpha$)-, Beta ($\beta$)- und Gamma ($\gamma$)-Bänder sowie spezielle Bänder wie $\mu$ und $\sigma$.\footnote{Vgl. \cite{nayak_waveforms}} Jedes Band wird mit bestimmten Gehirnzuständen oder Funktionen assoziiert. In der Praxis können diese Bänder von den rohen EEG-Daten durch Spektralanalysetechniken (z.B. Fourier-Transformationen oder Welch-Verfahren) extrahiert werden, was oft als quantitative EEG-Analyse bezeichnet wird. \footnote{Vgl. \cite{wikipedia_eeg_nyquist}} \newline
...
\subsubsection{Abgrenzung der Frequenzbänder voneinander}
\label{sec:frequenzbaender_kurz}
Die verschiedenen Frequenzbänder unterscheiden sich dabei aber nicht ausschließlich durch ihre Frequenz, auch die Amplitude, die Topographie und der assoziierte mentale und physiologische Zustand sind unterschiedlich. Die nachfolgende Tabelle stellt die verschiedenen Frequenzbänder anhand ihrer Unterscheidungsmerkmale gegenüber. \newline

\begin{table}[htbp]
\centering
\caption{Übersicht der physiologischen EEG-Frequenzbänder und ihrer Eigenschaften (nach Nayak \& Anilkumar, 2024)}
\label{tab:frequenzbaender}
\begin{tabular}{ll l p{4cm} p{5cm}}
\toprule
\textbf{Band} & \textbf{Frequenz (Hz)} & \textbf{Amplitude} & \textbf{Typische Topographie} & \textbf{Physiologischer Zustand} \\
\midrule
Delta ($\boldsymbol{\delta}$) & 0,5 -- 4   & Sehr hoch  & Frontozentral (Erw.) / Posterior (Kinder) & Tiefschlaf \\
Theta ($\boldsymbol{\theta}$) & 4 -- 7     & Hoch       & Frontozentral / Mittellinie               & Schläfrigkeit, Meditation \\
Alpha ($\boldsymbol{\alpha}$) & 8 -- 12    & Mittel     & Posterior und Okzipital-dominant            & Entspannung, Wachzustand, geschlossene Augen \\
Beta ($\boldsymbol{\beta}$)   & 13 -- 30   & Niedrig    & Frontozentral                       & Mentale Aktivität, Konzentration, Aufmerksamkeit \\
Gamma ($\boldsymbol{\gamma}$) & $> 30$     & Sehr niedrig & Diffus / Lokal (Kortikal)                 & Kognitive Verarbeitung \\
\bottomrule
\end{tabular}
\end{table}

Bezogen auf die Amplitude lässt sich mit steigender Frequenz eine Verringerung dieser feststellen. Wird die Topographie betrachtet, zeigt sich, dass Delta, Theta und Beta vermehrt frontozentral auftreten und Gamma diffus auftritt, während Alpha posterior und okzipital-dominant ist. 
...
\subsection{Der Alpha-Rhythmus (\textbf{\textasciitilde 8--13 Hz})}

Der Alpha-Rhythmus war die erste, von Hans Berger entdeckte, beschriebene EEG-Oszillation. Klassischerweise wird er als ausgeprägter posterior dominanter Rhythmus beobachtet, wenn eine Person wach, aber entspannt ist und beispielsweise ihre Augen schließt.\footnote{Vgl. Ebd.} Alpha-Aktivität hat typischerweise ihre maximale Amplitude über dem Okzipitallappen und wird durch das Öffnen der Augen oder mentale Anstrengung abgeschwächt, ein Phänomen, welches als \gls{berger-effekt} bezeichnet wird.\footnote{Vgl. \cite{jensen2010}} Im Wesentlichen zeigt der Alpha-Rhythmus einen ruhigen, erholsamen Zustand an, welcher verschwindet, wenn wir wachsam werden oder uns aktiv mit Gedanken beschäftigen. Neben dem posterioren Alpha gibt es andere Varianten, wie z.B. den Mu-Rhythmus über dem sensomotorischen Kortex, welcher mit motorischer Inaktivität assoziiert wird.\footnote{Vgl. Ebd.} Der Mu-Rhythmus wird während Bewegungen oder sogar der reinen Beobachtung von Bewegungen (in Verbindung mit der Aktivität von Spiegelneuronen) unterdrückt (Desynchronisation).\footnote{Vgl. \cite{jadeja2021}} Die Mu-Suppression wird häufig in BCI-Anwendungen zur Erkennung motorischer Vorstellungen oder Absichten genutzt. \newline 

% ANMERKUNG: "Vgl. Ebd." im ersten Satz dieses Absatzes bezieht sich auf die
% zuletzt zitierte Quelle (\cite{wikipedia_eeg_nyquist} aus 2.4.1) - inhaltlich
% würde hier vermutlich eher \cite{jadeja2021} passen (wie in Kapitel 1 bzw.
% eurer Quellensammlung zum Alpha-Rhythmus). Da eine Änderung hier eine
% inhaltliche/Quellen-Entscheidung wäre, habe ich NICHTS geändert - bitte
% selbst prüfen, ob "Ebd." hier tatsächlich die richtige Quelle referenziert.

Die korrekte Abbildung des Alpha-Rhythmus eignet sich besonders gut als Kriterium, um zwei EEG-Systeme miteinander zu vergleichen, da das Auftreten dieses aufgrund des Berger-Effekts sehr gut vorhersagbar ist. Konkret bedeutet das, dass beim Schließen der Augen ein Ansteigen der Alpha-Power zu erwarten ist und beim Öffnen der Augen ein Sinken dieser.\footnote{Vgl. Ebd.} Zusätzlich ist dieses Phänomen sehr gut untersucht und sehr einfach als Teil des Versuches durchzuführen (siehe Kapitel "Methodik").


\subsection{Bedeutung und Anwendungen der Frequenzbänder in BCI-Systemen}

Die Frequenzbandanalyse ist auch für viele Anwendungen von besonderer Bedeutung. Im klinischen Kontext werden EEG-Rhythmen genutzt, um Anomalien zu identifizieren und BCI-Forscher entwickeln Systeme zur gezielten Modulation bestimmter Bänder. Eine klassische Applikation ist das Nutzen der Alpha- oder Beta-Rhythmusmodulation für die BCI-Steuerung, wie beispielsweise, wenn eine Person lernt, ihre Alpha-Leistung zu steigern, um einen Befehl auszuführen, oder die Mu-/Beta-Suppression nutzt, um eine Prothese zu steuern. Studien zeigten, dass eine einfache binäre Kontrolle durch die Erkennung von Veränderungen im EEG erreicht werden kann. In einer frühen Demonstration erlernte ein gelähmter Patient, Beta-Aktivität zu nutzen, um einen Schalter zu bedienen, welcher einen Computer-Cursor bewegte und zusätzlich elektrische Stimulatoren für seine Hand aktivierte. Dadurch konnte der Patient ein gewisses Maß an Bewegungsfreiheit zurückgewinnen.\footnote{Vgl. \cite{uchida1991}} 


\subsection{Ereignisbezogene Potenziale (ERPs)}
\label{sec:erps}

\subsubsection{Grundprinzip und Abgrenzung zu spontanen Rhythmen}

Während EEG-Rhythmen den generellen Zustand des Gehirns widerspiegeln, sind ERPs neuronale Reaktionen, welche mit bestimmten sensorischen, kognitiven oder motorischen Ereignissen verbunden sind und aus dem gesamten EEG extrahiert werden können.\footnote{Vgl. \cite{luck2005}, S. 4} Da jede ereignisausgelöste neuronale Reaktion sich klein im Vergleich zu EEG-Schwankungen verhält, werden in den meisten EEG-Experimenten ERP-Wellen durch verschiedene Signalmittelungen von zufälligem Hintergrundrauschen isoliert.\footnote{Vgl. Ebd., S. 56}

\subsubsection{Abgrenzung der ERP-Komponenten voneinander}
ERP-Komponenten bestehen aus einer Serie von positiven und negativen Spannungsdeflexionen und werden anhand ihrer Polarität (P für positive oder N für negative Spannung relativ zur Basislinie (normale elektrische Gehirnaktivität eines Menschen im Ruhezustand)) sowie ihres typischen Zeitverlaufs gekennzeichnet. Die nachfolgende Tabelle soll einen Überblick über die Unterscheidungsmerkmale wie Latenz und Topographie der vier frühen visuellen ERP-Komponenten geben. \newline
...
Insgesamt zeigt sich, dass die Komponenten in der folgenden zeitlichen Abfolge: C1, P1, N1 und P2 auftreten und in einem Zeitfenster von 40--250 ms post stimulus auftreten. Auffallend ist auch, dass die ERP-Komponenten sich mit fortschreitender Zeit von posterioren Regionen in Richtung anteriorer Regionen des Schädels bewegen. Je später die Latenz, desto schwerer ist auch die Isolation der Komponenten, da sich die elektrischen Signale im Gehirn zunehmend überlappen.\footnote{Vgl. \cite{luck2005}, S. 35} \newline
Im nächsten Abschnitt werden vor allen Dingen die charakteristischen Merkmale der P1-Komponente betrachtet, da diese Komponente als weiteres Vergleichskriterium unserer Validierungsstudie dient. Alle weiteren ausführlichen Beschreibungen der anderen Komponenten finden sich im Anhang
\ref{app:erp}. \newline
...
Die positive P1-Komponente erreicht an den lateral okzipitalen Elektroden ihre maximale Amplitude und tritt 60--90 ms nach dem Reiz auf und erreicht ihr Maximum bei 100--130 ms. Sie ist allerdings schwierig genau zu identifizieren, da sie sich mit der C1-Welle überlappt. Zusätzlich ist anzumerken, dass sich die P1-Latenz und -Amplitude maßgeblich durch den Stimuluskontrast verändern können.\footnote{Vgl. Ebd., S. 36}\newline

Genau diese Eigenschaften der P1 sind ausschlaggebend für die Eignung als Validierungskriterium für unsere Studie: Erstens kann die Auswertung hauptsächlich auf die lateral okzipitalen Elektroden beschränkt werden, da die P1 ihre maximale Amplitude an diesen Elektroden erreicht, und zweitens lässt sich das Verhalten der P1 gegenüber Kontrast- und Größenabhängigkeit des Stimulus gut vorhersagen. Dies bedeutet in der Praxis, dass die P1, wenn ProbandInnen ein großes und kontrastreiches Bild sehen, eine geringere Latenz und größere Amplitude aufweisen müsste, als wenn sie ein kleines Bild mit geringem Kontrast sehen. 


\subsection{Einflussfaktoren und Störgrößen}
\label{sec:stoergroessen}
...
Ein BCI ermöglicht eine direkte Informationsübertragung zwischen einem organischen Gehirn und einem technischen System.
...
Die Funktionsweise eines BCI beruht auf der kontinuierlichen Erfassung physiologischer Signale, vor allem der elektrischen Aktivität des Gehirns.
Diese wird mithilfe eines Elektroenzephalogramms (EEG) gemessen.
Die Signale, die aufgezeichnet werden, werden mit vorab festgelegten oder erlernten Mustern verglichen, um charakteristische neuronale Signale festzustellen, die gleichzeitig als Kontrollsignale dienen.\footnote{Vgl. Ebd.}
...
Ein vereinfachtes Beispiel für ein BCI-System wurde im Rahmen eines Informatikprojekts in der 10. Klasse umgesetzt.
...
Im Rahmen der Seminarfacharbeit wird neben der Validierungsstudie eines portablen EEG-Systems zusätzlich die Möglichkeit zur Steuerung eines Roboterarms mithilfe eines BCI-Programms untersucht.
...
BCIs gewinnen in der heutigen Zeit an Bedeutung.
Insbesondere Menschen mit körperlichen Behinderungen können von der Technik mit der Umsetzung von BCIs profitieren.
Es gibt bereits prototypische Systeme, die beispielsweise die Steuerung eines Cursors, die Nutzung von Buchstabierprogrammen oder die Kontrolle von prothetischen Geräten ermöglichen. \footnote{Vgl. \cite{heuer2015}}
...
Unsere Motivation ist es, mit einem portablen System einen Roboter zu bewegen. Wir stellen uns die Frage:\newline
Ist die Genauigkeit eines portablen EEG-Systems hinreichend, um bestimmte Bewegungsabläufe eines Roboterarms zu initialisieren?\newline


Der erste Schritt zur Beantwortung dieser Frage ist die Validierung der Daten des portablen Gerätes, unabhängig vom Roboterarm. Die Validierung findet durch den Vergleich mit einem Profi-System aus der Forschung statt, dessen Daten Grundlage für andere Arbeiten sind, die Erkenntnisse liefern. Es wird deshalb davon ausgegangen, dass die Daten des Profi-Systems valide genug sind, um den Roboterarm zu steuern.
Wird festgestellt, dass die Daten des portablen Systems vergleichsweise sehr schlecht, also durch Rauschen und Artefakten so strak verfälscht, dass die Daten nicht mehr analysiert werden können, sind, kann die Fragestellung beneint werden. Ist dies nicht der Fall, kann auf Grundlage dieser Arbeit weitere Forschung in Hinsicht auf die Verbindung zum und Steuerung des Roboterarm durchgeführt werden.\newline
Die Forschungsfrage, mit der sich diese Arbeit also im Speziellen beschäftigt, ist:\newline
Ist die Qualität der Daten des portablen System ansatzweise mit denen des Profi-Systems vergleichbar?\newline


Als Paradigmen für den direkten Vergleich wurden die ERP-Komponente P1 und das Alpha-Frequenzband gewählt.
Bezüglich dieser Paradigmen wurden folgende Hypothesen aufgestellt:

\underline{P1-Paradigma}
\begin{itemize}
    \item Die Spannungsdeflexion der P1-Konponente ist unterschiedlich groß, je nachdem, ob ein kontrastreicher oder kontrastarmer Stimulus präsentiert wird. Dies ist bei beiden Systemen zu erwarten.
    \item Bei dem portablen System sind entweder gar keine oder nur sehr frühe Komponenten der ERPs, wozu die P1-Komponente gehört, zu erkennen.
\end{itemize}

\underline{Alpha-Paradigma}
\begin{itemize}
    \item Sortiert man die Proband:innen bezüglich ihrer gemessenen Alpha-Power, so ist die Rangfolge in beiden Systemen ähnlich. Es wird Abweichungen geben, aber große Teile der Folge sollten übereinstimmen.
\end{itemize}

\underline{Allgemein}
\begin{itemize}
    \item Bei dem Alpha-Paradigma wird eine höhere Ähnlichkeit der Daten zwischen den Systemen erkennbar sein als bei dem P1-Paradigma. Es sollte das Alpha-Paradigma genutzt werden, um den Roboterarm zu steuern.
    \item Bei dem portablen System ist das Signal-to-Noise-Verhältnis deutlich geringer. Das bedeutet, dass die Störungen im Verhältnis zu dem tatsächlichen Signal größer sind.
\end{itemize}
